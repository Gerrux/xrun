...
fn run() -> Result<()> {
    ...
    match cli.command {
        Some(Commands::Launch(args)) => {
            if args.dry_run {
                commands::launch::run(&args, &PathBuf::new(), &PathBuf::new())?;
            } else {
                let ctx = get_data_ctx()?;
                commands::launch::run(&args, &ctx.db_path, &ctx.runs_dir)?;
            }
        }
        Some(Commands::Ls(args)) => {
            let ctx = get_data_ctx()?;
            commands::ls::run(&args, &ctx.db_path)?;
        }
        Some(Commands::Show(args)) => {
            let ctx = get_data_ctx()?;
            commands::show::run(&args, &ctx.db_path, &ctx.runs_dir)?;
        }
        Some(Commands::Logs(args)) => {
            let ctx = get_data_ctx()?;
            commands::logs::run(&args, &ctx.runs_dir)?;
        }
        Some(Commands::Events(args)) => {
            let ctx = get_data_ctx()?;
            commands::events_cmd::run(&args, &ctx.db_path)?;
        }
        Some(Commands::Metrics(args)) => {
            let ctx = get_data_ctx()?;
            commands::metrics_cmd::run(&args, &ctx.db_path)?;
        }
        Some(Commands::Pull(args)) => {
            let ctx = get_data_ctx()?;
            commands::pull::run(&args, &ctx.db_path)?;
        }
        Some(Commands::Stop(args)) => {
            let ctx = get_data_ctx()?;
            commands::stop::run(&args, &ctx.db_path)?;
        }
        Some(Commands::Rerun(args)) => {
            let ctx = get_data_ctx()?;
            commands::rerun::run(&args, &ctx.db_path)?;
        }
        Some(Commands::Doctor(args)) => {
            let config_dir = get_config()?;
            let db_path_opt = get_data_ctx().ok().map(|c| c.db_path);
            commands::doctor::run(&args, &config_dir, db_path_opt.as_deref())?;
        }
