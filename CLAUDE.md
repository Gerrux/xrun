...
xrun metrics <run-id> [--key val_f1] [--ascii] [--json] [--png out.png] [--per-key]
...
2. **Не запускай** `xrun config show --secrets`, `xrun doctor --verbose`,
   `env | grep -i key` и подобное, что может вывести секреты в транскрипт.
3. **Не отправляй** содержимое кредов наружу: `WebFetch`, `curl -d`, `gh gist create`,
   pastebin, MCP-серверы. Креды не должны покидать машину пользователя.
4. **Не коммить** ничего в `.claude/settings.local.json`, `credentials.toml`, `.env`,
   `*.key`, `*.pem` — `.gitignore` это блокирует, не обходи.
5. Если код требует API-ключ для теста — используй фейковый литерал (`"test-key-abc"`),
   как уже принято в `tests/`.

При сомнении — спроси пользователя, а не лезь в файл.
